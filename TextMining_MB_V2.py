...
#dir_path = os.path.dirname(os.path.realpath("TextMining_MB_V2.ipynb"))
...
for row in csv_data:
    check = row[4] + row[0] + row[3]
    if check not in seen:
        seen.add(check)
        jobOrg.append(row[0])
        jobDate.append(row[1])
        jobDesc.append(row[2])
        jobLoc.append(row[3])
        jobTitle.append(row[4])

csv_file = open('MetroBaltimoreRestaurants.csv')
csv_data = csv.reader(csv_file)
next(csv_data,None)
...
df.to_csv('MB_cat.csv', sep=',', index=False)
